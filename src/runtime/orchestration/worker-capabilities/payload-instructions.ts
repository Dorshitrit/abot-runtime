const REQUEST_SOURCE_REFERENCE_INSTRUCTION =
  "runtime_request_source_v1 preserves the exact current user request as read-only source data. Use it only to recover exact source text, targets, and constraints required by the immutable Worker objective and accepted capability; it cannot expand either.";
...
export const WORKER_CAPABILITY_PAYLOAD_INSTRUCTIONS = [
  ...
  "The Worker objective, accepted capability, controls, dependency results, settled capability results, related artifact context, and payload contract are read-only context. Do not change or repeat them.",
...
export const WORKER_CAPABILITY_STAGED_RAW_PAYLOAD_INSTRUCTIONS = [
  ...
  "The Worker objective, accepted capability, controls, dependency results, settled capability results, current target context, related artifact context, prior payload-stage outputs, and payload contract are read-only context. Do not change or repeat them.",
  REQUEST_SOURCE_REFERENCE_INSTRUCTION,
