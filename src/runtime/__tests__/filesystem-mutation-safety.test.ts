...
// On Linux, exercise the isolated directory backend as well as procfs. This
// tests real I/O through the macOS dispatch path; it is not a native Mac run.
const testBackends =
...
describe.each(testBackends)(
  "filesystem mutation safety (%s backend)",
  (platform) => {
    beforeEach(() => {
    ...
    test("rejects a resolved read redirected through an intermediate symlink", async () => {
      ...
      const target = fixture.runtimePathResolver.resolve(
        "read-safe/target.txt",
        {
          requirePath: true,
