...
describe("filesystem Agent Plugin", () => {
  let tempRoot = "";
  let agentWorkDir = "";
  let workspaceDir = "";
  ...
  test("projects the canonical manifest, payload stages, adapters, and skills", () => {
    ...
    expect(Object.keys(extension.capabilities)).toEqual(FILESYSTEM_TOOL_NAMES);
  ...
  test("bounds file and directory observations with explicit metadata", async () => {
    ...
    const fileResult = await plugin.handlers.read_file!({ path: "large.txt" });
  ...
  test("applies one scoped edit and refuses invalid structured drafts", async () => {
    ...
    await writeFile(path.join(agentWorkDir, "notes.txt"), "alpha\nbeta\ngamma");
